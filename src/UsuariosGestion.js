...
import api from './api';
import { UserPlus, Shield, Mail, Trash2, Users, Lock, Key, AlertTriangle } from 'lucide-react';
...
const UsuariosGestion = () => {
  ...
  const eliminarUsuario = async (id, nombre) => {
    // Protección para el usuario root o admin principal
          ...
          <div style={{overflowY: 'auto', maxHeight: '400px'}}>
              ...
              <tbody>
                  ...
                  {usuarios.length === 0 && !cargando && (
                      <tr><td colSpan="3" style={{textAlign:'center', padding:'20px', color:'#94a3b8'}}>No hay otros usuarios registrados.</td></tr>
                  )}
              </tbody>
              </table>
          </div>
...
const styles = {
  container: { maxWidth: '1100px', margin: '30px auto', padding: '0 20px', fontFamily: 'Inter, system-ui, sans-serif' },
  ...
  layoutGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '25px' },
  card: { background: 'white', padding: '25px', borderRadius: '16px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)', border: '1px solid #f1f5f9' },
  ...
  input: { padding: '10px 14px', borderRadius: '8px', border: '1px solid #e2e8f0', fontSize: '14px', outline: 'none', transition: 'border 0.2s' },
  btnSave: { gridColumn: 'span 2', background: '#0f172a', color: 'white', padding: '14px', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: 'bold', marginTop: '10px', fontSize: '13px', letterSpacing: '0.5px' },
  table: { width: '100%', borderCollapse: 'collapse' },
  trHeader: { borderBottom: '2px solid #f1f5f9', textAlign: 'left' },
  tr: { borderBottom: '1px solid #f8fafc', transition: 'background 0.2s' },
  th: { padding: '12px', color: '#94a3b8', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '1px' },
  ...
  badgeUser: { background: '#f0f9ff', color: '#075985', padding: '4px 10px', borderRadius: '20px', fontSize: '10px', fontWeight: '900' },
